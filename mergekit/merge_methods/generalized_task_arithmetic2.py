...
class GTATask(Task[torch.Tensor]):
    ...
    def execute(
        ...
        **_kwargs,
        ...
        layer_name = self.weight_info.name
        base_tensor = tensors[self.base_model]

        # Skip Token embeddings layer and lm_head
        if layer_name in ['model.embed_tokens.weight', 'tok_embeddings.weight', 'lm_head.weight', 'output.weight']:
            # Set base model's density and weight as close to 0 as possible
            density = 0.0
            weight = 0.0
            base_tensor.mul_(density)
            base_tensor.mul_(weight)
            return base_tensor
